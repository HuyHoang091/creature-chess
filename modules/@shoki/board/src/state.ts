...
import {
	getPiecesWithoutIds,
	getPiecePositionsWithoutIds,
} from "./utils/filter";

export const createInitialBoardState = <TPiece>(
	id: string,
...
export const createBoardSlice = <TPiece extends HasId>(
	id: string,
	size?: { width: number; height: number }
): BoardSlice<TPiece> => {
	const {
		...
		reducers: {
			setBoardSizeCommand: (
				...
				return {
					...state,
					size: { width, height },
					piecePositions: Object.entries(state.piecePositions).reduce<{
			...
			setPieceLimitCommand: (
				state,
				{ payload: limit }: PayloadAction<number | null>
			) => ({
				...state,
				pieceLimit: limit,
			}),
			...
			setBoardPiecesCommand: (
				state,
				{
					payload: { pieces, piecePositions, size: newSize },
				}: PayloadAction<{
					pieces: PiecesState<TPiece>;
					piecePositions: PiecePositionsState;
					size?: { width: number; height: number };
			...
			addBoardPieceCommand: (
				...
				{
					payload: { x, y, piece },
				}: PayloadAction<{ x: number; y: number; piece: TPiece }>
			) => ({
				...state,
				pieces: {
			...
			moveBoardPieceCommand: (
				...
				const piece = state.pieces[pieceId] as TPiece | undefined;
			...
			removeBoardPiecesCommand: (
				state,
				{ payload: pieceIds }: PayloadAction<string[]>
			) => ({
				...state,
				pieces: getPiecesWithoutIds(state.pieces, pieceIds),
				piecePositions: getPiecePositionsWithoutIds(
					state.piecePositions,
					pieceIds
				),
			}),
			updateBoardPiecesCommand: (
				state,
				{ payload: pieces }: PayloadAction<TPiece[]>
			) => {
			...
			swapPiecesCommand: (
				state,
				{ payload: { aId, bId } }: PayloadAction<{ aId: string; bId: string }>
			) => {
				if (!aId || !bId) {
					return state;
				}
				...
				return {
					...
					piecePositions: {
						...state.piecePositions,
						[aPosition]: (b as TPiece).id,
						[bPosition]: (a as TPiece).id,
					},
				};
			},
		},
