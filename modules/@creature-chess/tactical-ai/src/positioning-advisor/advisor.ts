...
export class PositioningAdvisor {
  ...
  async getAdvice(
    ...
    const hasPotential = potentialEnemyBoard && Object.keys(potentialEnemyBoard.pieces).length > 0;
    console.log(`[Advisor] getAdvice called — myBoard pieces=${Object.keys(myBoard.pieces).length}, enemyBoard pieces=${Object.keys(enemyBoard.pieces).length}${hasPotential ? `, potentialEnemyBoard pieces=${Object.keys(potentialEnemyBoard.pieces).length}` : ''}`);
    ...
    console.log(`[Advisor] Generated ${allScenarios.length} enemy scenarios (${realScenarios.length} real + ${potentialScenarios.length} potential), trialsPerScenario=${trialsPerScenario}`);
    ...
    for (let i = 0; i < this.config.numFormations * 2 && generatedCandidates.length < this.config.numFormations; i++) {
