...
export const getPlayerInventory = (state: PlayerState): string[] =>
	state.playerInfo.inventory;

export const getPlayerPieceItems = (state: PlayerState): Record<string, string[]> => {
	const allPieces = [
		...BoardSelectors.getAllPieces(state.board),
		...BoardSelectors.getAllPieces(state.bench),
	];
	...
	for (const piece of allPieces) {
		if (piece.items?.length) {
			map[piece.id] = piece.items.map((item) => item.itemId);
		}
	}
	return map;
};
