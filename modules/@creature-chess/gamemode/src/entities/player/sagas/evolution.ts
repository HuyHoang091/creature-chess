...
export const evolutionSaga = function* () {
	...
	>(
		...
		function* ({ payload: { piece } }) {
			...
			// Collect all items from ALL pieces being merged
			const allConsumedPieces = [
				...matchingBoardPieces,
				...matchingBenchPieces,
				piece,
			];
			const collectedItems = allConsumedPieces.reduce<typeof piece.items>(
				(acc, p) => {
					if (p.items && p.items.length > 0) {
						for (const item of p.items) {
							acc.push({ itemId: item.itemId });
						}
					}
					return acc;
				},
