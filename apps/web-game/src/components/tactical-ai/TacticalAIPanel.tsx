...
const TacticalAIPanel: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = React.useState<"positioning" | "coach">("positioning");
  const [loading, setLoading] = React.useState(false);
  const [positioningResult, setPositioningResult] = React.useState<PositioningAdvice | null>(null);
  const [coachMessages, setCoachMessages] = React.useState<Array<{ role: "user" | "ai"; text: string }>>([]);
  ...
  const handleCoachSend = async () => {
    ...
    try {
      ...
      if (lower.startsWith("/build") || lower.startsWith("/team") || lower.startsWith("/doi")) {
        ...
        await requestCoachAdviceStream(
          ...
          { traits: myTraits, pieces: myPieces.map((p) => ({ name: p.definition?.name || "", definitionId: p.definitionId })) },
          (chunk) => {
      ...
      if (lower.startsWith("/item") || lower.startsWith("/đồ") || lower.startsWith("/do")) {
        const targetPiece = selectedPieceId
          ? myPieces.find((p) => p.id === selectedPieceId)
          : myPieces[0];
        if (!targetPiece) {
          setCoachMessages((prev) => [...prev, { role: "ai", text: "Không tìm thấy quân nào để gợi ý đồ. Vui lòng chọn một quân trên bàn." }]);
          setLoading(false);
          return;
        }
        const role = targetPiece.definition?.traits?.[0] || "general";
        const currentItems = (targetPiece as any).itemIds || (targetPiece as any).items || [];
        setCoachMessages((prev) => [...prev, { role: "ai", text: "" }]);
        await requestCoachAdviceStream(
      ...
      if (lower.startsWith("/counter") || lower.startsWith("/khắc") || lower.startsWith("/khac")) {
      ...
      if (lower.startsWith("/scout") || lower.startsWith("/đối") || lower.startsWith("/doi")) {
        const enemyPieces = matchBoard
          ? BoardSelectors.getAllPieces(matchBoard).filter((p) => p.ownerId !== localPlayerId)
          : [];
        if (enemyPieces.length === 0) {
      ...
      await requestCoachAdviceStream(
        raw,
        { pieces: myPieces.map((p) => ({ name: p.definition?.name || "", definitionId: p.definitionId })) },
        (chunk) => {
          setCoachMessages((prev) => {
            const last = prev[prev.length - 1];
            if (!last || last.role !== "ai") return prev;
      ...
      setCoachMessages((prev) => [...prev, { role: "ai", text: `Lỗi: ${e.message}` }]);
    } finally {
      setLoading(false);
    }
  };
      ...
      {open && (
        <div className={styles.panel}>
          ...
          <div className={styles.tabs}>
            <button
              className={activeTab === "positioning" ? styles.tabActive : styles.tab}
            ...
            {activeTab === "coach" && (
              <div className={styles.chatContainer}>
                <div className={styles.chatMessages} ref={chatMessagesRef}>
                  {coachMessages.length === 0 && (
                    <div className={styles.emptyChat}>
                      Gõ /help để xem lệnh nhanh, hoặc hỏi tự do về đội hình, item, chiến thuật...
                    </div>
                  )}
                ...
                <div className={styles.chatInputRow}>
                  ...
                  />
                  <button className={styles.sendBtn} onClick={handleCoachSend} disabled={loading}>
                    Gửi
                  </button>
                </div>
              </div>
