const canvas = document.getElementById("arena-scene");
const context = canvas.getContext("2d");
const theme = document.body.dataset.theme || "water";
...
function drawEnergyVein(time, offset, colorA, colorB, alpha) {
...
function drawCentralGlow(time) {
  ...
  context.strokeStyle = themeConfig.ringB.replace("ALPHA", `${0.07 + Math.sin(time * 0.0018) * 0.02}`);
  context.lineWidth = 2;
  context.beginPath();
  context.arc(x, y, radius * 1.24, 0, Math.PI * 2);
  context.stroke();

  context.strokeStyle = themeConfig.ringA.replace("ALPHA", `${0.08 + Math.sin(time * 0.0014) * 0.025}`);
  context.lineWidth = 1;
  context.beginPath();
...
function maybeTriggerLightning() {
  ...
  if (Math.random() > themeConfig.lightningChance) {
    return;
  }
  ...
  for (let i = 0; i < segments; i += 1) {
    ...
    if (i > 1 && Math.random() > 0.55) {
      const branch = [[currentX, currentY]];
      let bx = currentX;
      let by = currentY;
...
function drawLightning() {
  ...
  context.save();
  context.globalCompositeOperation = "screen";
  context.strokeStyle = themeConfig.lightningStroke.replace("ALPHA", lightningAlpha);
  context.shadowColor = themeConfig.lightningShadow.replace("ALPHA", lightningAlpha);
  ...
  context.fillStyle = themeConfig.lightningFlash.replace("ALPHA", lightningAlpha * 0.08);
  context.fillRect(0, 0, width, height);
  context.restore();
...
function drawMotes(time) {
  for (const ember of embers) {
    const x = ember.x * width + Math.sin(time * 0.0007 * ember.drift + ember.sway) * 24;
